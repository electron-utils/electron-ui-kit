...
function _resize (
  ...
  for ( let i = 0; i < elementList.length; ++i ) {
    ...
    if ( el.tagName === 'UI-SPLITTER-RESIZER' ) {
      continue;
    }
...
class SplitterResizer extends window.HTMLElement {
  constructor () {
    super();
  ...
  _onMouseDown ( event ) {
    ...
    // add drag-ghost
    if ( platform.isWin32 ) {
      utils.addDragGhost( this.vertical ? 'ew-resize' : 'ns-resize' );
    } else {
      utils.addDragGhost( this.vertical ? 'col-resize' : 'row-resize' );
    }
...
module.exports = SplitterResizer;
